...
function setPValue(val, mx, desc, obj, al) {
    ...
    $(".progFill", $(obj)).animate({width: (($(obj).width() / mx) * val)}, 500);
...
function setPValue2(val, mx, desc, obj, al) {
    ...
    $("#p", _).animate({width: (_nw > _ow ? _ow : _nw)}, 500).css({
...
function setPValue3(val, mx, desc, obj, al) {
    ...
    $("#p", _).animate({width: (_nw > _ow ? '100%' : _nw + "%")}, 500);
    /*
    if(_.data('dir') == 'l') {
        $('#t', _).css({right: 11});
    } else {
...
function sairVila() {
    $.ajax({
        url: '?acao=mapa_vila&e', type: 'get', dataType: 'script', async: false
    });
}

function mapaVila() {
    $.ajax({
        url: '?acao=mapa_vila&v', type: 'get', dataType: 'script', async: false
    });
}
    ...
    $("div.ex_tooltip").each(function () {
        ...
        $("#" + t.attr("title")).on('mouseover', function () {
            ...
            if (parseInt(_.data('tooltip-float'))) {
                ...
                t.css({
                    position: 'absolute', top: offset.top + _.height() + 5, left: offset.left
                }).show();
            } else {
                if (_.data('no-anim')) {
                    t.show();
                ...
                if ($(this).css('position') == 'absolute' || parseInt($(this).data('absolute'))) {
                    t.css({
                        left: $(this).position().left + $(this).width() + 5, top: $(this).position().top
                    });
...
function createTimer(h, m, s, t, f, identifier, change_title) {
    ...
    var _t = setInterval(function () {
        ...
        if (t instanceof Array) {
            for (var ii in t) {
                $(t[ii]).html((h < 10 ? "0" + h : h) + ":" + (m < 10 ? "0" + m : m) + ":" + (s < 10 ? "0" + s : s));
            }
        } else {
...
function jalert(m, t, k, w) {
    ...
    var win = bootbox.dialog({
        message: m, title: t, buttons: [{
            'label': 'Fechar', callback: function () {
...
window.jconfirm = function (msg, title, ok_callback, cancel_callback, options) {
    ...
    var win = bootbox.dialog({
        message: msg, title: title, buttons: [{
            'label': 'Fechar', callback: function () {
                if (cancel_callback) {
        ...
        setInterval(function () {
            $('.button, button').each(function () {
                ...
                if (navigator.appName.match(/opera/i)) {
                    _.css({
                        width: _.width(), height: _.height(), display: 'block', float: 'left'
                    });
                }
        ...
        $.ajax({
            ...
            success: function (result) {
                if (result.success) {
                    ...
                    Swal.fire({
                        icon: 'error', title: 'Erro ao criar sua conta', html: '<ul>' + errors.join('') + '</ul>'
                    });
                }
            }
        });
